...
export default function PostPage() {
  ...
  const handleShare = async () => {
    const currentUrl = `${window.location.origin}/post/${getPostId()}`
    if (navigator.share) {
      try {
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <Card className="mb-8 overflow-hidden shadow-xl border-0 bg-gradient-to-br from-card to-card/50">
          ...
          <CardContent className="p-8">
            ...
            <div
              className="prose prose-lg max-w-none dark:prose-invert prose-headings:text-foreground prose-p:text-foreground prose-strong:text-foreground prose-a:text-primary hover:prose-a:text-primary/80 prose-code:bg-muted prose-code:px-2 prose-code:py-1 prose-code:rounded prose-pre:bg-muted prose-pre:p-4 prose-pre:rounded-lg"
